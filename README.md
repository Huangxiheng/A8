...
│   ├── logger.ts               # 日志框架（winston）+ axios 请求/响应日志拦截器
│   └── index.ts                # 模块导出入口
...
基于 [winston](https://github.com/winstonjs/winston) 日志框架实现，支持通过环境变量配置输出目标和级别：
...
- 输出目标通过 `.env` 配置文件控制，支持控制台、文件、两者同时输出

#### 日志配置

日志行为通过项目根目录的 `.env` 文件配置（参考 `.env.example`）：

| 配置项             | 说明                                              | 可选值                                              | 默认值            |
| ------------------ | ------------------------------------------------- | --------------------------------------------------- | ----------------- |
| `LOG_FILE_PATH`    | 日志文件路径（相对路径基于项目根目录）            | 任意路径                                            | `./logs/app.log`  |
...
日志输出格式：`[YYYY-MM-DD HH:mm:ss.SSS] [LEVEL] 消息内容`
